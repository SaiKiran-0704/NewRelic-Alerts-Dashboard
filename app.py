...
def categorize_alert(row):
    ...
    infra_keywords = ['cpu', 'memory', 'disk', 'storage', 'network', 'host', 'server', 'load balancer', 'latency', 'k8s', 'db', 'gcp']
...
if not st.session_state['alert_data'].empty:
    ...
    with t1:
        st.dataframe(df_main[df_main['Category'] == 'Infra'][cols].style.map(style_status_column, subset=['Status']), use_container_width=True, hide_index=True, column_config=common_config)
    with t2:
        st.dataframe(df_main[df_main['Category'] == 'SOC'][cols].style.map(style_status_column, subset=['Status']), use_container_width=True, hide_index=True, column_config=common_config)
else:
